...
function history($path, $action, $content)
{
	...
	{
		$PreviousQuery = mysql_query("Select `Content` from `Wiki_Edits` where `ID` < '$action[1]' order by `ID` desc limit 1");
		...
		$previousQuery = mysql_query("Select `ID`, `Title`
										from `Wiki_Edits`
										where `PageID` = '$pageID' and `ID` > '$action[1]'
											order by `ID` limit 1");
...
//			$PageEditTime = date("F j\, Y G:i:s", $PageEditTime)." EST";
	...
	else
	{
		...
		foreach($Data as $Result)
		{
			...
			$content['Body'] .= "<tr $Class><td style='min-width:$minWidth;'>$HistoryTime</td><td>$HistorySize</td><td>$historyTags</td><td><b><a href='/names?id=$AccountID'>$HistoryName</a></b></td><td style='max-width:400px;'><span style='float:right;'><a href='$DiffURL' rel='nofollow'>d</a></span><b><a href='$HistoryURL' rel='nofollow'>$HistoryTitle</a></b></td><td>$HistoryDescription</td></tr>";
		}
