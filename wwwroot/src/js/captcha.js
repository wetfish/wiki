...
function captcha() //the actual captcha 'class' 
{
    ...
    {
        var request = new Request('/src/captcha-assets/'+assetName); //initialize request
    ...
    function retrieveChallenge() //retrieve generated image data
    {
        ...
        challengeXHR.open('GET', "/src/captcha.php?new='true'", true); //will call captcha.php with no parameters
    ...
    function checkSuccess(first) //ping server for success state
    {
        if(user.usingNet || first){ //only calls if user is actively attempting solution
            ...
            successXHR.open('GET', "/src/captcha.php?x="+user.x+"&y="+user.y, true); //will call captcha.php with user data
